    ...
    while(stck->size > 0){
        ...
        if (curr_route->nCitiesVisited == nTotalCities){
            ...
            for (int i = curr_route->nCitiesVisited; i < nTotalCities; i++){
                ...
                double newLength = curr_route->length + distanceTable[curr_route->path[curr_route->nCitiesVisited-1]][curr_route->path[i]];
...
//     RouteDefinition *bestRoute;
//     
//     if (route->nCitiesVisited == nTotalCities)
//     {
//             // If all grain bags have been placed we simply add the distance to get back home (closing the loop)
//         route->length += distanceTable[route->path[route->nCitiesVisited-1]][route->path[0]];
//         return route;
//     }
//     
...
//         // position route->nCitiesVisited). To do this, we copy 'route' to
//         // newRoute and swap the bags in position 'i' and route->nCitiesVisited
//         // in newRoute.
//     for (int i = route->nCitiesVisited; i < nTotalCities; i++)
//     {
//             // newlength is the current route length + the path to bag number 'i'.
//         double newLength = route->length + distanceTable[route->path[route->nCitiesVisited-1]][route->path[i]];
//         RouteDefinition *newRoute; 
//         
...
//         
//             // Swaps the position of bag # 'i' and bag # 'nCitiesVisited' from route
//         newRoute->path[route->nCitiesVisited] = route->path[i];
//         newRoute->path[i]              = route->path[route->nCitiesVisited]; 
